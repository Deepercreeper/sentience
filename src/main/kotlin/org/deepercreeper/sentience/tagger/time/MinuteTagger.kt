...
class MinuteTaggerConfig(symbolService: SymbolService) : SimpleTaggerConfig({ MinuteTagger(it, symbolService) })

class MinuteTagger(document: Document, symbolService: SymbolService) : AbstractValueTagger<Int>(document, KEY, symbolService) {
    override val maxLength get() = 2

    override fun mappings() = (0..9).asSequence().map { it.toString() }.map { it to it }

    override fun convert(text: String) = text.toIntOrNull()?.takeIf { it in 0..59 }

    companion object {
        const val KEY = "minute"
    }
}